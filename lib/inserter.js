...
async function contentInserter(content1,content2,options) {
    ...
    if (options.overwrite || tagSection.length == 0) {
        ...
        for (let i = 0 ; i < tagSection.length ; i++) {
            ...
            let score = similarityScore(old,updateContent,options);
        ...
        logger.debug(`max_score = ${max_score} , requested ${options.similarity}`);
...
function similarityScore(oldText,updatedText,options) {
    let _old = oldText;
    let _new = updatedText;

    if (options.similarityNormalization) {
        _old = htmlNormalization(_old);
        _new = htmlNormalization(_new);

        logger.debug(`normalized-old: ${_old}`);
        logger.debug(`normalized-new: ${_new}`);
    }

    return stringSimilarity(_old,_new);
}

function htmlNormalization(str) {
    return str.replaceAll(/<[^>]+>/g,"").replace(/^\s*/g,"").replace(/\s*$/g,"");
}
